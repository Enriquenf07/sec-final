...
class Service:
    ...
    @staticmethod
    def cadastrar_funcionario(form): 
        password = form['password'].encode()  
        hashed = bcrypt.hashpw(password, bcrypt.gensalt())
        if('login' in form and 'email' in form):
            existing_user = User_repository.find_user_by_login(form['login'])
            if existing_user:
                return "Login já existe."
        form['password'] = hashed.decode()
        user_id = User_repository.create_user(form, current_user.company)
        User_repository.set_role(user_id, form['role'])
        send_email(
            to=form['email'],
            subject="Cadastro realizado com sucesso",
            contents=f"Olá {form['name']}, seu cadastro foi realizado com sucesso na nossa plataforma.",
        )
        return None
    
    @staticmethod
    def find_all_by_empresa(com_id):
        return User_repository.find_all_by_empresa(com_id)
